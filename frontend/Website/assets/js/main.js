...
// Local Configuration Filter Parameters
let currentCategory = "all";
let currentFormat = "all";
let searchString = "";

const cards = Array.from(document.querySelectorAll(".course-style-card"));
...
function filterLibrary() {
  cards.forEach(card => {
    ...
    const matchesFormat = currentFormat === "all" || cardFormat === currentFormat;
    const matchesSearch = cardTitle.includes(searchString.toLowerCase());

    if (matchesCategory && matchesFormat && matchesSearch) {
...
// Action Button Listeners Mapping Injection
document.querySelectorAll(".btn-view").forEach(btn => {
  btn.addEventListener("click", () => {
    alert(`Opening digital viewer layer interface window for: ${btn.dataset.target}`);
  });
});

document.querySelectorAll(".btn-download").forEach(btn => {
  btn.addEventListener("click", () => {
    alert(`Initializing native pipeline data stream transfer protocol download for: ${btn.dataset.target}`);
  });
});

// Dropdown Action Event Hookups
document.getElementById("categoryDropdown").addEventListener("change", (e) => {
  currentCategory = e.target.value;
  filterLibrary();
});

document.getElementById("formatDropdown").addEventListener("change", (e) => {
...
// Primary System Initializer Initialization Entry-Point
